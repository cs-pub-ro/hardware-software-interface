...
In addition to the buffer, we have also allocated a local variable of 4 bytes, accessible at the address `rbp - 4`.
...
```Assembly
mov dword [rbx + TODO], TODO
```

> Do this after the buffer initialization sequence (after the `jl fill_byte` instruction on line 47).
